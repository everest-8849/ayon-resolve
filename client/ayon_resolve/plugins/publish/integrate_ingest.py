...
class IngestArgs:
    def __init__(self, metadata, push, push_only, video, origin, force, fps, output_dir=None):
    ...
    def __str__(self):
        return (
            f"IngestArgs(metadata={self.metadata}, push={self.push}, "
            f"video={self.video}, origin={self.origin}, force={self.force}, "
            f"fps={self.fps}, output_dir={self.output_dir})"
        )
