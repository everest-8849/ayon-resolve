...
class ExtractShotsIngest(publish.Extractor):
    ...
    def process(self, instance):
        # Find the remapped OTIO file and the intermediate video
        editorial_pkg_repre = None
        intermediate_repre = None

        for repre in instance.data.get("representations", []):
            if repre["name"] == "editorial_pkg":
                editorial_pkg_repre = repre
            if repre["name"] == "intermediate":
                intermediate_repre = repre

        if not editorial_pkg_repre:
            raise ValueError(
                "No 'editorial_pkg' representation found on instance.")

        if not intermediate_repre:
            raise ValueError(
                "No 'intermediate' representation found on instance.")

        staging_dir = Path(editorial_pkg_repre["stagingDir"])
        otio_remap_file = staging_dir / editorial_pkg_repre["files"]
